...
# Material parameters with dielectric constant driving the physics
material_params = {
...
# Reference dielectric constant for scaling (SiO2)
kappa_ref = 3.9
...
def calculate_temperature_current(T, material, gate):
    ...
    Vt = Vt_base * gate_param['eta']

    # Drive current scaling: proportional to Cox (i.e. kappa)
    drive_scale = cox_ratio
    # Leakage scaling: lower band-offset materials leak more at subthreshold
    ...
    if T == 77:  # Cryogenic: Step function (quantum behavior)
        ...
        if gate == 'Pi-Gate':
            I_exp1 = 1e-9 * leak_scale * np.exp(8 * np.maximum(0, Vgs_eff))
            I_exp2 = 1e-10 * drive_scale * np.exp(12 * np.maximum(0, Vgs_eff - 0.05))
            Ids = I_exp1 + I_exp2 * np.exp(-1 * np.maximum(0, Vgs_eff))
        else:
            I_exp1 = 1e-11 * leak_scale * np.exp(12 * np.maximum(0, Vgs_eff))
            I_exp2 = 5e-12 * drive_scale * np.exp(18 * np.maximum(0, Vgs_eff - 0.15))
...
print("Temperature-dependent IV plot generated with distinct characteristics for each temperature and material.")
